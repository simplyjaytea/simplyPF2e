...
**Next task:** obtain permission before any authenticated GitHub write, then use branch + PR. After release/deployment, verify new characters start at derived full HP, grounded choices/native grants still work, and the post-creation review renders correctly on the VPS test world. Never infer permission to push from this roadmap.
