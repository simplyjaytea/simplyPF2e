...
**Unverified in a live world**: the updated native PF2e character grant chain (blocked on PR #70 deployment); local-provider setup and full actor creation in Foundry; focus spells (PC + NPC); Free Archetype; Int languages; PC runes; PC gold spend; the six new presets; and the **entire item forge**. The production request path itself has been browser-integration-tested cross-origin for CORS/preflight, SSE, compatibility retry, a bound bearer key, and an unbound key being withheld. A real Ollama `qwen3.8:latest` service also passed production `/models` discovery from a separate browser origin, the production 64-token connection check with exact usage, and direct production creature/PC/item-concept requests (26, 27, and 10 parsed fields respectively); Foundry compendium grounding, UI, and document creation remain the local-provider gap.

**Next task: deploy and complete live verification.** Merge the green PR #70 with a backup, update the Foundry test world, and re-run the Dwarf character grant path. Then validate one local-provider actor flow and the Item Forge's first live item creation.
