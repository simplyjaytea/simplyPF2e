...
**Latest local feat-slot correction:** Free Archetype candidates remain `class` feats for source filtering, but PF2e 8.4.1's `CharacterFeats` creates a distinct `archetype` group with `archetype-<level>` slots. The final resolver retains the variant marker and the builder now writes that exact group location rather than colliding with an ordinary `class-<level>` slot. Pure slot and production creation-path regressions pass; an independent review checked the 8.4.1 group source. This is placement fidelity only, not a claim that feat prerequisites are comprehensively automated.
