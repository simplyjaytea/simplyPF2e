...
Git/API reconciliation on 2026-08-31: `origin/main` is `07a369f` (PR #83), latest release **v0.3.5.43**. The audit-fix stack merged in PR #77; shared UI updates, documented live QA, partial ChoiceSet automation, and PC named-loot budget/cross-bucket dedup followed in PRs #79–#82. The one-click/class-path/prerequisite milestone is now published; the release-workflow `lang/en.json` validation gap remains open. See [HANDOFF.md](HANDOFF.md) for the active branch and [HISTORY.md](HISTORY.md) for the preserved audit/QA record.
...
**Published one-click milestone:** PR #83 merged the `codex/one-click-grounding` implementation into `main` as `07a369fc` and published release **v0.3.5.43**. The UI now has Monster, NPC, Encounter, and Player Character intents; the primary Generate action creates after generation, while Preview Plan remains no-write. Monster/NPC share the existing PF2e NPC pipeline but receive distinct intent guidance. New creature drafts clear before sheet presentation, and partial encounter creation is rolled back best-effort. Exact candidate identity, completion manifests, full source preflight, hardened PC completion, exact native class-path staging, and the conservative PC prerequisite gate are now in the public artifact. Live Foundry QA and the next implementation slice remain open.
...
**Latest local class-path staging:** before Actor creation, the module can clone a level-one native class bridge (Rogue's Racket or Methodology) from an enabled exact Class Features pack, link it to the class, and remove only that class clone's matching `system.items` entry; all other native class grants remain untouched. Its dynamic tag query is narrowed to exact issued class-feature candidates. Only candidates with static, non-predicated direct choices and no descendant choice path qualify; required selections are preselected through PF2e's real GrantItem mechanism or creation stops before the actor exists. The complete-only registry remains Fighter-only: ordinary PC feat prerequisites still cannot be evaluated against a staged actor, so this bridge is deliberately not a basis for widening Rogue/Investigator. Wizard also remains excluded pending class-owned spellbook/curriculum support. Local mocks and source inspection do not replace live Foundry grant-chain QA.

**Latest local prerequisite gate:** `getFeatCandidates({requireNoPrerequisites:true})` now offers only feats whose real `system.prerequisites.value` is an explicit empty array. This conservative gate prevents dependent, malformed, or unreadable feats from entering the complete PC path. Empty entitlements are retained so the completion manifest blocks rather than silently dropping a slot; NPC and legacy callers remain permissive. This is not an evaluator for ordinary prerequisites and does not widen the Fighter-only registry. The next implementation slice is the staged-actor evaluator, followed by independent review and live Rogue/Investigator grant-chain QA.

**Post-release handoff:** PR #83 and release v0.3.5.43 are complete. The topic branch remains available as a recoverable implementation record; future changes must start from `origin/main` and use a new `codex/` branch plus PR. Do not mutate the VPS without explicit deployment/QA authorization. The next implementation slice is the fail-closed ordinary feat prerequisite evaluator against a staged actor, followed by independent schema review and only then Rogue/Investigator qualification. Execute HANDOFF's skill-completion and preceding HP/choice/spell/signature acceptance checklist on the VPS before claiming the public artifact is fully live-verified.
...
- **Free Archetype prerequisite graph is unbuilt** — level-2+ complete one-click generation stops before provider spend because PF2e feat prerequisites are display text, not a general eligibility API. Its native `archetype-<level>` slot placement is now exact.
