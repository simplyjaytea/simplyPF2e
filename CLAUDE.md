...
**PC class qualification correction:** the complete-only registry now offers Fighter alone. PF2e 8.4.1 class data shows Rogue's Racket, Investigator Methodology, Arcane School, and Arcane Thesis are mandatory class-grant choices. The native ABC grant path re-fetches those feature sources, outside the current direct/one-level ChoiceSet preselection seam; offering those classes as one-click complete would be false. They remain unavailable until the module stages and resolves that grant graph exactly.
