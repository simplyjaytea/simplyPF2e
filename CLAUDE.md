...
**Paused one-click roadmap:** `codex/one-click-grounding` is paused at local HEAD `1d9d755`. The UI now has Monster, NPC, Encounter, and Player Character intents; the primary Generate action creates after generation, while Preview Plan remains no-write. Monster/NPC share the existing PF2e NPC pipeline but receive distinct intent guidance. New creature drafts clear before sheet presentation, and partial encounter creation is rolled back best-effort. Exact candidate identity, completion manifests, full source preflight, hardened PC completion, exact native class-path staging, and the conservative PC prerequisite gate are implemented locally. Live QA and publication remain open.
...
**Paused handoff:** do not push, open a PR, merge, release, deploy, or mutate the VPS without explicit authorization. When work resumes, first implement the fail-closed ordinary feat prerequisite evaluator against a staged actor, obtain the required independent schema review, and only then consider qualifying Rogue/Investigator. After an authorized release, execute HANDOFF's skill-completion and preceding HP/choice/spell/signature acceptance checklist on the VPS test world. Never infer publication permission from this roadmap.
