...
- **Focus-spell support** (PC + NPC) is built across two stacked branches (`feat/pc-focus-spells`, `feat/npc-focus-spells`) plus a docs commit — all real-source-verified, all logic-level self-checked, **none of it live-tested and none of it merged**. This is the top open item.
- PC generator's prior bug-fix rounds (background feat slots, wealth, rarity cap, compendium-match badges) are logic-self-checked but still not confirmed in an actual running Foundry world.
- Item forge (all 3 phases) remains completely unverified end-to-end in a live game.
- **Next natural task**: a real live-Foundry test — either the focus-spell branches, a PC build (feat slots/wealth/rarity cap/badges), or the item forge's first live item creation, whichever comes first. `pc-builder.mjs` is the first file to check if PC generation misbehaves; `builder.mjs`'s NPC `createActor()` for creature-side issues.
