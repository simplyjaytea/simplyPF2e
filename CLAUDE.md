...
| `choice-set.mjs` | Pre-answers supported static PF2e `ChoiceSet` rules on direct items and one level of `GrantItem` sources. ABC-generated features, deeper grants, and unresolved/dynamic choices can still open native prompts. Its existing first-option fallback is console-warned; do not expand preference guessing. Zero deps, node-testable. |
...
**Next task:** finish the reviewed local PC-creation fix, obtain permission before any authenticated GitHub write, then use branch + PR. After release/deployment, verify new characters start at derived full HP and native choices/grants still work on the VPS test world. Never infer permission to push from this roadmap.
