...
**Latest local loadout slice:** `pc-loadout.mjs` runs after native item/grant creation, when the new actor's prepared martial proficiencies are authoritative. It mirrors PF2e 8.4.1's category/group/base/equivalent/synthetic weapon and category/synthetic armor rank selection, then only changes exact equipment created by this transaction: one proficient armor and at most two held hands. Untrained/conflicting equipment stays stowed and is surfaced in the post-create review. The item state write remains in the new-actor rollback boundary. Ammunition loading, investment, containers, and individual activation requirements are not faked by raw updates and remain open loadout work. All 56 regression files, script syntax checks, module/localization JSON parsing, and whitespace checks pass; this has no live Foundry proof yet.
