...
**Published baseline:** PR #106 merged at `6f82c30` and released **v0.3.5.66**, following the UI overhaul in #105. Setup updated only SimplyPF2e and relaunched the existing world on Foundry 14.365 / PF2e 8.5.0. Follow-up generation controls, PC response validation, and new-run rendering are reviewed on `codex/consumer-generation-controls` and await publication. Detailed evidence and acceptance limits are in [docs/consumer-readiness-2026-09-08.md](docs/consumer-readiness-2026-09-08.md). Git remains authoritative; [HANDOFF.md](HANDOFF.md) tracks current work and [HISTORY.md](HISTORY.md) holds prior evidence.
...
- **Verification:** the published consumer UI passes 85 regressions and 120 script syntax checks on Node 22.23.2, with independent Astra slice audits (final status in HANDOFF.md). The current follow-up passes **86 regression files and 121 script syntax checks**, plus JSON/duplicate-key and whitespace checks, with independent Astra slice approval. The final Generator failure-token accounting slice is approved: rejected AI usage is retained once under the active/originating stage label. PR and release gates check syntax, regressions, and JSON before publication; GitHub actions use v5 with Node 22 for project checks. Local tests establish deterministic behavior, not native Foundry rendering or grant-chain acceptance.

**Recorded live evidence:** native ABC grants passed on v0.3.5.34; subsequent v0.3.5.39 QA covered cloud creation, heightened NPC spells, PC equipment deduction, and passive/activated items. v0.3.5.56 passed concurrent cancellation isolation, runed weapon/armor price-level parity, and healing/condition macro escaping on Foundry 14.365 / PF2e 8.5.0. v0.3.5.60 passed the courier preview's scroll labels and 19/19 match display; a separate selector call confirmed explicit feat omission, but no omission-bearing full UI run was recorded. v0.3.5.63 covered directory/kind switching, Ghost Touch weapon and Slick armor price-level parity, and mundane NPC one-click creation. **v0.3.5.66 is the latest installed baseline:** provider probe/draft retention and active close/reopen passed, but NPC no-gear and weapon Forge trials failed closed before document writes. On .66 the short Forge IDs pass weapon/armor native price-level parity and healing-charm creation. NPC creation succeeds with full HP but the no-gear/no-loot text request is ignored by the provider, motivating authoritative UI switches. PC QA uncovered and locally fixed an ABC ID/name validator mismatch; a same-length reused progress card could leave stale controls and completion visible, now fixed by one initial full render per run. The reviewed controls, ABC, rendering, and failure-token accounting follow-up remains unpublished and pending a module-only release/update; preserve these limits when reporting coverage.
