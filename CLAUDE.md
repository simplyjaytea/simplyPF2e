...
**Published baseline:** PR #104 merged at `89f9a35`; the GitHub API confirms **v0.3.5.64**. The consumer UI branch is locally reviewed and awaiting its authorized PR publication, then a module-only Foundry update. Detailed evidence and acceptance limits are in [docs/consumer-readiness-2026-09-08.md](docs/consumer-readiness-2026-09-08.md). Git remains authoritative; [HANDOFF.md](HANDOFF.md) tracks current work and [HISTORY.md](HISTORY.md) holds prior evidence.
...
- **Verification:** the published baseline has 82 regression files; the consumer UI branch passes 85 regressions and 120 script syntax checks on Node 22.23.2, with independent Astra slice audits (final status in HANDOFF.md). PR and release gates check syntax, regressions, and JSON before publication; GitHub actions use v5 with Node 22 for project checks. Local tests establish deterministic behavior, not native Foundry rendering or grant-chain acceptance.
