...
> **Status: in review (PR [#49](https://github.com/simplyjaytea/simplyPF2e/pull/49)), not yet released or verified against a live Foundry world.** The mechanism below is built and passes static checks, but the exact field names it writes for character build data (ability boosts, skill ranks, spellcasting proficiency) are asserted from training knowledge, not confirmed against the installed pf2e system's real schema. Don't rely on it in play until that's verified — see [Known limitations](#known-limitations).

Switch the toggle to **Player Character**, describe a concept ("a grizzled dwarf ranger who hunts undead"), set a level (1–20), and generate. Unlike NPC mode, nothing here is scale-word math — a PC is built from real Ancestry, Background, and Class compendium items plus chosen feats at every level slot (ancestry/class/skill/general, per the Core Rulebook's leveling cadence) and ability boosts, and the PF2e system's own derived-data engine computes AC, HP, saves, and proficiencies once those real items are embedded, the same way it would for a character built by hand on the sheet.

Every choice is grounded the same way spells and equipment already are for NPCs: the AI drafts first-draft ancestry/background/class/feat/spell names as inspiration, then a grounded pass matches each against your real compendium and the AI picks only from what actually exists — nothing is invented except loot (which follows the exact same free-form exception NPCs already have for coins and scrolls). Starting gear and wealth are grounded and budgeted the same way NPC equipment and loot are, scaled to the character's own level instead of an NPC's encounter share.
...
- **Player Character mode is unverified against a live Foundry world.** The field names it writes for character-build data (ability boosts, skill proficiency ranks, spellcasting proficiency) are asserted from training knowledge about the pf2e system schema, not confirmed against real actor data — no running world with the pf2e system was available while building it. Before trusting a generated character's numbers, open its sheet and sanity-check AC/HP/saves/proficiencies against what its ancestry/background/class/level should produce; if something looks off, that assembly code is the first place to check. Single-class builds only — no multiclass archetypes, and there's no pre-create screen to swap individual AI picks (regenerate for a different build instead).
...
- [ ] **Full PC-power-level characters** — in review, PR [#49](https://github.com/simplyjaytea/simplyPF2e/pull/49): a new Player Character generator mode builds real single-class characters (Ancestry/Background/Class/feats/spells/gear, all compendium-grounded) and lets the PF2e system compute stats from the real embedded items — see [Player Character mode](#player-character-mode). Not checked off yet: pending live verification of the character-build field names before merge/release.
