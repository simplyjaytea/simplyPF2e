...
- Implementation is published as **v0.3.5.70** through PR #110. Parent records publication and native evidence on `codex/npc-automatic-abilities-qa`; Luna owns the module-only Setup update and one focused level-15 Stunning Blows generation. Exact next step: finish installation/native inspection, record results and limits, reset the temporary viewport, and commit the QA record locally without a documentation-only release.
- User requested Astra implementation and independent Astra audits, then cheaper delegation to conserve usage. Astra implemented the three core slices; Luna completed bounded rollback/deduplication/documentation work and owns routine installation/QA. All reported Astra findings are closed. Final local gate passed **88 regression files**, **124 syntax checks**, JSON/duplicate-key, whitespace and release file selection. No further broad audit or provider matrix unless a concrete new failure warrants it.
- Luna verified that NPC selection already scans enabled class-feat sources across classes and prioritizes exact draft names before the 16-entry cap. Empty catalogs, selection errors, or all-invalid nonempty replies retain draft names; final exact matching blocks. No raw reply/catalog snapshot is available to identify this run's precise branch.
