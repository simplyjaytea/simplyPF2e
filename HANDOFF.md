...
- No GitHub write, provider request, module update, Foundry/world mutation, or deletion of user artifacts occurred. Publishing this cleanup requires the user's authorization under AGENTS.md; do not push or merge main directly.
...
The cleanup is verified and ready as a local commit on `codex/repo-cleanup`. Any later authorized publication must use this branch + a PR and verify CI and the automatic release. Recheck installed state before resuming the outstanding forge UI acceptance; preserve existing QA artifacts.
